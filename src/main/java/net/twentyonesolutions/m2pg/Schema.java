...
//        String tgtTable = tgtSchema + "." + table.name;
        ...
        try {
            ...
            statTgt.execute("ALTER TABLE "+ tgtTable +  " DISABLE TRIGGER ALL;");
...
//                    throw ex;
            ...
            if (rowCount == 0){
                ...
                if (table.hasIdentity()) {
                    ...
                    if (rs.next()) {
                        ...
                        //这边是改过源码的
                        String sequenceName = tgtTable + "_" +config.getTargetColumnName(identity.name) +"_seq";
                        sequenceName = sequenceName.replaceAll("\"","");
                        sequenceName = sequenceName.replaceAll("public.", "");
                        sequenceName = "public." + "\""+ sequenceName + "\"";

                        //alter sequence public."AbpUserLoginAttempts_Id_seq" restart with 389000;
                        String sqlRecommended = "ALTER sequence " +
                                sequenceName  +
                                " RESTART WITH " +
                                recommendValue +
                                ";";
            ...
            if (hasErrors) {
                ...
                statTgt.execute("ALTER TABLE "+ tgtTable +  " ENABLE TRIGGER ALL;");
